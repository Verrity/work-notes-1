...
4. Собрать прошивку двумя путями
	...
	2. Пересобрать полностью
			...
			Полностью:
			<u>`clear; make unconfig; yes | make clean-all; make config-wlc30; make all && esr-validator ~/esr/esr-base/images/arm64/wlc30.firmware`</u>
...
apps:
```bash unfold
clear; make apps-all && make firmware && esr-validator ~/esr/esr-base/images/arm64/wlc30.firmware
```
all:
```bash unfold
