...
# Надо удалить все`libvirtd` пакеты
Они создают мосты ovirt которые почему-то ломают передачу TCP.
# Установка
...
***Чтобы загрузиться с полного снимка нужно выделять для VM столькоже памяти и ядер, сколько было на снимке!!*** 
